...
def test_hash_fn_plain_int_single_widget(class_):
...
@pytest.mark.parametrize("class_", [W.IntSlider, W.BoundedIntText])
def test_hash_fn_plain_int_multiple_widgets(class_):
...
@pytest.mark.parametrize("class_", [W.IntRangeSlider])
def test_hash_fn_plain_IntRangeSliderModel_single(class_):
...
@pytest.mark.parametrize("class_", [W.IntRangeSlider])
def test_hash_fn_plain_IntRangeSliderModel_multiple(class_):
...
@pytest.mark.parametrize("class_", IW.BOOLEAN_CONTROL_WIDGETS)
def test_hash_fn_plain_multiple_bool(class_):
    w1 = class_()
    w2 = class_()
    w_states = [w.get_state() for w in [w1, w2]]
...
@pytest.mark.parametrize("class_", IW.SELECTION_SINGLE_CONTROL_WIDGETS)
def test_hash_fn_plain_selection_single_value(class_):
    options = ["a", "b", "c"]
...
def test_hash_fn_plain_selection_multi_value(class_):
    options = ["a", "b"]
    w = class_(options=options)
    w_states = [w.get_state() for w in [w]]
